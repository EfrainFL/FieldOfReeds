...
#BadGuy's Turn
def BGTurn(TurnCount, atk, dmg, heroHP, BGHP):			#Pass Turn Count, atk, and dmg to BGTurn() function,
	...
	return (TurnCount, atk, dmg, heroHP, BGHP)		#Returns the Turn Count, atk, and dmg variables for future use

#Calculates the remaining HP for Hero and BadGuy	
def calcHP(atk, dmg, heroHP, BGHP):				#Pass HP for Hero and BadGuy to calcHP() function
	heroHP -= dmg						#Reduce Hero HP by damage done by BGTurn() (dmg)
	BGHP -= atk						#Reduce BadGuy HP by damage done by heroTurn() (atk)
	return (atk, dmg, heroHP, BGHP)				#Return HP Totals for use in determining Exit value
